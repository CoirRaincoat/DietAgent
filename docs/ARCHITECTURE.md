...
Planner 排除标记为甜品、饮料或加工组件的候选，优先保留仍符合新约束的旧菜，并在空位上考虑类别与做法多样性。它是确定性的启发式组合器，不宣称全局最优营养优化。其他用餐者只能通过本轮表达补充共享限制，人数不等于已经加载多份档案。
...
SQLite 默认位于 `runtime/sessions.sqlite3`。会话包括用户、版本号、累计约束、当前菜单 ID、有效标志、未解决澄清和最近 12 条对话。每个会话通过进程内异步锁串行处理；数据库采用短事务与版本条件检查，模型调用期间不持有数据库事务。
