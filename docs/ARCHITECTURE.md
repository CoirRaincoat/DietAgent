...
| `app/agent/menu_balance.py` | 基于类别、做法和明确冷热文字证据计算内部搭配排序，并渲染对外事实说明 |
...
Planner 排除标记为甜品、饮料或加工组件的候选，优先保留仍符合新约束的旧菜。新增或必须替换的槽位按字典序优先覆盖蔬菜类菜、蛋白质来源菜和主食，再比较类别覆盖、做法多样性、明确冷热证据、规则软分及稳定来源顺序；搭配分不能抵消过敏、忌口、菜数或汤数硬约束。对于尚未覆盖的食材偏好，再尝试增加菜单级覆盖的局部交换，保留已经覆盖的偏好、汤数和指定替换范围，并优先选择搭配损失较小的候选。它是确定性的有界启发式组合器，不保证全局最优营养。仅健康排序分变化不触发既有菜单重排。其他用餐者只能通过本轮表达补充共享限制，人数不等于已经加载多份档案。

套餐结构分是规划器和测试 harness 使用的内部确定性信号，不进入领域响应模型、API JSON 或前端类型，也不作为面向评委的“系统自评”。冷热分类只有明确冷食词或已识别热加工方式才计入，其他保持 unknown；份量缺失时不计算荤素重量比例。服务把程序生成的类别数量、做法及冷热证据摘要作为解释阶段必选事实，因此 DeepSeek 不能补造或改写这些事实。
...
下一阶段可在既有边界上扩充审核词典与定量营养来源、独立评测断言、多人成员状态与撤销操作；再按实测错误引入新的检索实现，按耗时结果优化模型与缓存。多 worker、鉴权、请求恢复和部署监控属于服务化所需工作。
